...
datasets = [f'./datasets_phase2/dataset_{i}/alarm.csv' for i in range(4, 7)]
pri = [f'./datasets_phase2/dataset_{i}/causal_prior.npy' for i in range(4, 7)]

rca = []
rca.append(None)
for i in range(5, 7):
    rca.append(f'./datasets_phase2/dataset_{i}/rca_prior.csv')

topo = []
topo.append(None)
topo.append(f'./datasets_phase2/dataset_5/topology.npy')
topo.append(None)
# pc_paths = [f'./DAGMA/DAGMA_phase2/thres_0.05/dataset_{i}_graph_matrix.npy' for i in range(4, 7)]
# SAVEPATH = "./DAGMA_PTHP"
def do_PC():  # 一直跑不出来

    SAVE_PATH = [f'./init_matrix_phase2/PC_Result/dataset_{i}_graph_matrix.npy' for i in range(4, 7)]
    ...
    for data_path in datasets:
        ...
        a = np.load(pri[k])
        est_matrix = Utils.filter(prior_matrix=a, wait_to_filter_matrix=est_matrix)

        np.save(SAVE_PATH[k], est_matrix)
        print(f"Saved sucessfully in {SAVE_PATH[k]}!")
        print(f"----------------------------------------------dataset_{k+1+3}--OK---------------PC-----------------")


def do_SAM():
    print("SAM is running")
    #SAVE_PATH = [f'./init_matrix_phase2/SAM_result/bin_form/dataset_{i}_graph_matrix.npy' for i in range(4, 7)]
    SAVE_PATH_w = [f'./init_matrix_phase2/SAM_result/weighted_form/dataset_{i}_graph_matrix.npy' for i in range(4, 7)]
    k = -1
    ...
    for data_path in datasets:
        ...
        print(f"weighted saved sucessfully in {SAVE_PATH_w[k]}!")
        for threshold_ in range(0, 60):  # 设置一个阈值
            threshold = threshold_ / 100

            binary_W_est = (weighted_est_matrix > threshold).astype(int)
            print("Binary W_est:\n", binary_W_est)
            P = f"./init_matrix_phase2/SAM_result/bin_form/thres_{threshold}/dataset_{k+4}_graph_matrix.npy"
            Utils.check_path(P)

            a = np.load(pri[k])
            binary_W_est = Utils.filter(prior_matrix=a, wait_to_filter_matrix=binary_W_est)

            np.save(P, binary_W_est)
            print("Saved successfully Binary!")

def do_Notears():  # 一直跑不出来
    print("Notears is running")
    SAVE_PATH = [f'./init_matrix_phase2/Notears_result/dataset_{i}_graph_matrix.npy' for i in range(4, 7)]
    ...
    for data_path in datasets:
        ...
        np.save(SAVE_PATH[k], est_matrix)
        print(f"Saved sucessfully in {SAVE_PATH[k]}!")
        print(f"----------------------------------------------dataset_{k+1+3}--OK---------------Notears-----------------")
...
def do_PTHP_single(d: str, p: str, r: str, t: str, para: dict, pc: str, savedir: str):  # 处理单个数据集,传入路径
    ...
    cd = CD(origin_data_path=d, prior_imformation_path=p, rca_prior_path=r, topology_path=t, pthp_para=para,
            PC_re_path=pc, savedirpath=savedir)
    est_matrix = cd.PTHP()
...
def do_one_PTHP(task: int):
    ...
    do_PTHP_single(d=datasets[task], p=pri[task], r=rca[task], t=topo[task], para=para, pc=pc_paths[task],
                   savedir=save_dir)  # , pc = pc_paths[task])
