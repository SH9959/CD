...
class PTHP(BaseLearner):  # 添加了一个数据集的名字参数，用来保存数据集，具体更改在79,96,248行
    ...
    def __init__(self, topology_matrix, prior_matrix, init_result_matrix=None, delta=0.1, epsilon=1,
        ...
        self._init_result_matrix = init_result_matrix  # 添加PC_result
        if init_result_matrix is None:
            self._init_result_matrix = np.ones_like(self._prior_matrix)
    ...
    #             if self._dataname == 1 and num_iter == 62:
    #                 # 每2代保存一下
    #                 # logging.info(f'edge_mat:{edge_mat}')
    #                 pa = os.path.join(self._save_dir_path, f"PTHP_results", f"dataset_{self._dataname}_graph_matrix.npy")
    ...
    def _hill_climb(self):
        ...
        result = self._em(edge_mat)
        l_ret = result[0]
        BEST_ITERATIONS = [62, 72, 48, 14]
        for num_iter in range(self._max_iter):
            
            logging.info('[iter {}]: likelihood_score = {}'.format(num_iter, l_ret))
            if num_iter == BEST_ITERATIONS[self._dataname - 1]:

                pa = os.path.join(self._save_dir_path, f"_results", f"dataset_{self._dataname}_graph_matrix.npy")
                ...
                logging.info(f"iter[{num_iter}]:saved-----------in---------------{self._save_dir_path}------------------")
                logging.info(f"--------------------------dataset{self._dataname} finished!------------------------------")
            stop_tag = True
    ...
    def check_prior(self):
        return [(i, j) for i, j in product(range(len(self._event_names)),
                           range(len(self._event_names))) if self._prior_matrix[i][j] == -1 and self._init_result_matrix[i][j] == 1]
